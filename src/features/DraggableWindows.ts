export function initDraggableWindows() {
  ...
  draggableWindows.forEach((element) => {
    const header = element.querySelector<HTMLElement>(".draggable-header");
    if (!header) {
...
function onMouseDrag(event: MouseEvent, element: HTMLElement) {
  ...
  element.style.left = String(leftValue + event.movementX) + "px";
  element.style.top = String(topValue + event.movementY) + "px";
}
