...
export class D3Pack extends D3Visualization {
  ...
  constructor(layer: SVGGElement) {
    ...
    this.packLayout = d3.pack<Taxon>().padding(3);

    this.gNodes = this.layer
      .append("g")
  ...
  public async update(
    _event?: MouseEvent,
    _source?: d3.HierarchyNode<Taxon>,
    duration = 300,
  ): Promise<void> {
    if (!this.root) {
      return;
    ...
    const rootPacked = this.packLayout(
      this.root
        .sum((d) => {
          return this.computeGenomeRecursiveValue(d);
        })
        .sort((a, b) => {
    ...
    const nodeSel = this.gNodes
      .selectAll<SVGGElement, (typeof nodesData)[number]>("g")
    ...
    nodeEnter
      .append("circle")
      .attr("r", 0)
      .each(function (this: any, d, i, nodes) {
        const nodeEl = nodes[i];
        if (!nodeEl) return;
        const sel = d3.select(nodeEl as any);
        ...
        // Hover
        sel
          .on("mouseenter.packstyle", (event: any) => {
    ...
    nodeEnter
      ...
      .attr("opacity", 0)
      .text((d) => {
        return this.truncateLabel(d.data.name, 2 * d.r);
      });

    const nodeMerge = nodeEnter.merge(nodeSel as any);

    nodeMerge.transition(transition as any).attr("transform", (d) => {
    ...
    (nodeMerge.select("circle") as any as d3.Selection<SVGCircleElement, any, any, any>)
      ...
      .attr("fill", (d) => {
        return this.computeCircleStyle(d).fill;
      })
      .attr("fill-opacity", (d) => {
        return this.computeCircleStyle(d).fillOpacity;
      })
      .attr("stroke", (d) => {
        return this.computeCircleStyle(d).stroke;
      })
      .attr("stroke-opacity", (d) => {
        return this.computeCircleStyle(d).strokeOpacity;
      })
      .attr("stroke-width", (d) => {
        return this.computeCircleStyle(d).strokeWidth;
      });

    // Text aktualisieren / Sichtbarkeit (Leaf + Radius Threshold)
    nodeMerge.each(function (this: any, d, i, nodes) {
      const nodeEl = nodes[i];
      if (!nodeEl) return;
      const g = d3.select(nodeEl as any);
  ...
  private computeCircleStyle(d: any): {
    fill: string;
    fillOpacity: number;
    stroke: string;
    ...
    const inQuery = this.getQuery()?.some((t) => {
      return t.id === d.data.id;
    });
  ...
  private computeGenomeRecursiveValue(t: Taxon): number {
    ...
    if (!gc) {
      return 1;
    } // Minimalwert damit jeder Taxon einen Kreis erhält
    let sum = 0;
