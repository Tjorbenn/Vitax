...
export class NeverAPI {
   ...
   */
  public async getTaxonByName(name: string, exact = true): Promise<Taxon> {
  ...
  public async getTaxaByIds(taxonIds: number[]): Promise<Set<Taxon>> {
    ...
    const response = await request.Send();
    return this.MapResponseToTaxa(response);
  }
   ...
   */
  public async getSuggestionsByName(
    name: string,
    page = 1,
    pageSize = 10,
    exact = false,
  ...
  public async getSubtreeByTaxonId(taxonId: number): Promise<TaxonomyTree> {
    ...
    const response = await request.Send();
    const taxa = await this.getFullTaxaByIds(
      response
        .filter((entry) => {
   ...
   */
  public async getLineageFromTaxonIds(
    ancestorId: number,
    descendantId: number,
  ...
  private async ResponseToFullTaxa(response: Never.Response): Promise<Set<Taxon>> {
    if (
      ...
      return await this.getFullTaxaByIds(taxonIds.filter((id) => id !== -1));
    }
  }
